...
export async function makeMessage(guild: Guild, user: User): Promise<BaseMessageOptions> {
    ...
    if (user.startDate != null) {
        embed.addFields(
            {name: 'Online', value: `<t:${Math.floor(user.startDate.getTime() / 1000)}:R>`, inline: true}
        ).setTimestamp(Math.floor(user.startDate.getTime()));
    }

    if (user.gameDate != null) {
        embed.addFields(
            {
                name: 'Aktuelles Spiel',
                value: `**${user.gameName}** *<t:${Math.floor(user.gameDate.getTime() / 1000)}:R>*`,
                inline: true
            }
        )
    }
